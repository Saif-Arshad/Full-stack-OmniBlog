    ...
    const fetchData = async () => {
      try {
        const response = await fetch(
          `/api/filterblog?filter1=${urlDataone}&filter2=${urlDatatwo}`,
          {
            cache: 'no-store',
          }
        );
        ...
        <div className="bg-white dark:bg-slate-900 main-hero">
          {data.slice(0, load).map((blog) => (
            <div key={blog._id} className="main-card">
              ...
              <div className="p-5">
                <span className="text-lg capitalize font-bold text-purple-700 dark:text-blue-600">
                  {blog.categorie}
                </span>

                <Link href={`/blog/${blog._id}`}>
                  <h5 className="mb-2 text-lg sm:text-xl  hover:text-blue-500 hover:underline capitalize font-semibold mt-4 tracking-tight text-gray-900 dark:text-white">
                    {blog.title}
                  </h5>
                </Link>
                <Link href={`/blog/${blog._id}`}>
                  <p className="mb-3 font-normal text-gray-700 dark:text-gray-400">
                    {blog.maincontent.length > 150
                      ? `${blog.maincontent.substring(0, 90)}...`
                      : blog.maincontent}
                  </p>
                </Link>
                <div className="flex flex-wrap gap-1 mt-6 items-center text-purple-800 dark:text-blue-700 font-semibold">
